...
void fourthExercise(){
  ...
  employees.forEach((e){
    print('El nombre del empelado es ${e.name} y las horas trabajadas son ${e.WorkHours} y el total devengado es ${e.calculateWorkHours(valueWorkHours)} \n');
  }
  );
}
...
void fifthExercise(){
  ...
  print("El triangulo rectangulo con el cateto a de longitud $sideA y el cateto b de longitud $sideB tiene como hipotenusa ${sqrt(pow(sideA, 2) + pow(sideB, 2))}");
}

void sixthExercise(){
  print('Ingresa la temperatura en celsius');
  double? temperature = double.parse(stdin.readLineSync()!);
  print('La temperatura de celsius a fahrenheit es de ${(temperature*1.8)+32}');
}